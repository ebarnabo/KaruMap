            ...
            if (coordonnes_correct(jardin.latitude, jardin.longitude)) {
                ...
                if (!jardins_existant || jardins_existant.length === 0) {
                    ...
                    db.query(sql, jardin, (err, result) => {
                        if (err) throw err;
                        console.log(`Type de jardin ajouté à la base : ${jardin.type}`);
            ...
            if (monument.latitude !== null && monument.longitude !== null) {
                ...
                if (!Monuments_existant || Monuments_existant.length === 0) {
                    ...
                    db.query(sql, monument, (err, result) => {
                        if (err) throw err;
                        console.log(`Type de monument ajouté à la base : ${monument.type}`);
...
app.get('/marqueurs', async (req, res) => {
    try {
        const jardins = await queryDatabase('SELECT latitude, longitude, commune, code_postal, nom_du_jardin AS nom, \'jardins\' AS type FROM Jardins WHERE latitude IS NOT NULL AND longitude IS NOT NULL AND latitude != 999.9999999 AND longitude != 999.9999999');
        const monuments = await queryDatabase('SELECT latitude, longitude, commune, cp AS code_postal, nom_du_monument AS nom, \'SitesEtMonuments\' AS type FROM SitesEtMonuments WHERE latitude IS NOT NULL AND longitude IS NOT NULL AND latitude != 999.9999999 AND longitude != 999.9999999');
...
app.put('/marqueurs/:nom', async (req, res) => {
    ...
    const { new_nom, new_code_postal, new_commune } = req.body;

    try {
        console.log('marqueur à modifier:', nom);
        console.log('infos:',  new_nom,  new_code_postal, new_commune);
        let result = await queryDatabase('UPDATE Jardins SET nom_du_jardin = ?, code_postal = ?, commune = ? WHERE nom_du_jardin = ?', [new_nom, new_code_postal, new_commune, nom]);
        if (result.affectedRows === 0) {
            result = await queryDatabase('UPDATE SitesEtMonuments SET nom_du_monument = ?, cp = ?, commune = ? WHERE nom_du_monument = ?', [new_nom, new_code_postal, new_commune, nom]);
        }
        if (result.affectedRows === 0) {
